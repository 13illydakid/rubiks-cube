...
const MenuOptions = props => {
        ...
        const baseOptions = <>
           {props.state.cubeDimension<=optionLimitCP?<button id="ColorPicker" key="Color Picker" data="Color Picker" onClick={optionClick} className="leftButton">Color Pickerer</button>:<button className="leftButton invis"></button>}
           {props.state.cubeDimension<=optionLimitSOLVER?<button id="Solver" key="Solver" data="Solving" onClick={optionClick} className="leftButton">Solver</button>:<button className="leftButton invis"></button>}
        ...
        function algoStart(e,props){
            ...
            // console.log(generated.tempArr);
            generated.preSolveArray = [];
            algorithms.forEach(e=>{
                if(e.moves&&e.name===algo&&e.worksFor.includes(cD)) {
                  algoSet.push(...e.moves.split(" "));
                }
            })
            //console.log(algoSet);
            // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!----------------------------------!!!!");
            props.setState({activeAlgo:algo,moveSet:[...algoSet],rubiksObject : generated.preSolveArray,solveable:true,solvedSet:[...algoSet],solvedSetIndex:0,prevSet:[],autoPlay:false,autoRewind:false,autoTarget: false,playOne : false,jumpToEnd:false},
            //   () => {
            //     if (props.state.moveSet.length === 0) {
            //         let rubiksObject = props.state.rubiksObject;
            //         console.log(rubiksObject);
            //     }
            // }
            );
        }
