...
const cube = {
        ...
        middleRestructure.forEach((layer, index) => {
          ...
          for (let i = 0; i <= middleEdgeLength; i++) {
            for (let j = 0; j <= middleEdgeLength; j++) {
              if ((i === index || middleEdgeLength - i === index) && (j >= index && j <= middleEdgeLength - index)) {
                // pushes top row of ring
                ((j === 0 + index || j === middleEdgeLength - index) && ((j < Math.ceil(middleEdgeLength / 2) - 1) || (j > Math.ceil(middleEdgeLength / 2) + (size % 2 ? 1 : 0))))
                  ? middleCorners.push(tempMiddles[k][middleIndex]) : layer.push(tempMiddles[k][middleIndex]);
              }
              else if ((i > index && middleEdgeLength - i > index) && (j === index || j === middleEdgeLength - index)) {
  ...
  rotatePoint: function (c1, c2, direction, p1, p2, rotation) {
    ...
    return {
      p1: (Math.cos(theta) * (p1 - c1) - Math.sin(theta) * (p2 - c2) + c1),
